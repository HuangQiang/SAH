...
def draw_time_f1_H(dataset, q_time, f1_scores, k, y1_min, y1_max, y1_int, y1_scale, y2_min, y2_max, y2_int):
    ...
    plt.savefig('../figures/%s_H_k%s_time.pdf' % (dataset, k), format='pdf')


if __name__ == "__main__":
    k = 10
    t, f, max_t, min_f = read_csv_time_f1("results/Automotive/varying_H_k%s.tsv" % k)
    print(max_t, min_f)
    draw_time_f1_H(datasets[0], t, f, k, 3, 4, 0.2, 2, 90, 100.5, 2)

    t, f, max_t, min_f = read_csv_time_f1("results/CDs/varying_H_k%s.tsv" % k)
    print(max_t, min_f)
    draw_time_f1_H(datasets[1], t, f, k, 0.6, 1.6, 0.2, 2, 85, 100.5, 3)

    t, f, max_t, min_f = read_csv_time_f1("results/MovieLens/varying_H_k%s.tsv" % k)
    print(max_t, min_f)
    draw_time_f1_H(datasets[2], t, f, k, 0, 0.8, 0.2, 0, 90, 100.5, 2)

    t, f, max_t, min_f = read_csv_time_f1("results/Music100/varying_H_k%s.tsv" % k)
    print(max_t, min_f)
    draw_time_f1_H(datasets[3], t, f, k, 1, 4, 0.5, 3, 85, 100.5, 3)

    t, f, max_t, min_f = read_csv_time_f1("results/Netflix/varying_H_k%s.tsv" % k)
    print(max_t, min_f)
    draw_time_f1_H(datasets[4], t, f, k, 0, 6.4, 1, 0, 90, 100.5, 2)
