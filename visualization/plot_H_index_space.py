...
def draw_index_space_H(dataset, index_time, index_space, k, y1_min, y1_max, y1_int, y1_scale, y2_min, y2_max, y2_int, y2_scale):
    ...
    plt.savefig('../figures/%s_H_k%s_index.pdf' % (dataset, k), format='pdf')


if __name__ == "__main__":
    k = 10
    t, s, max_t, max_s = read_csv_index_space("results/Automotive/varying_H_k%s.tsv" % k)
    print(max_t, max_s)
    draw_index_space_H(datasets[0], t, s, k, 3, 5, 0.5, 2, 0, 1.6, 0.4, 3)

    t, s, max_t, max_s = read_csv_index_space("results/CDs/varying_H_k%s.tsv" % k)
    print(max_t, max_s)
    draw_index_space_H(datasets[1], t, s, k, 0, 16, 4, 0, 0, 30, 5, 0)

    t, s, max_t, max_s = read_csv_index_space("results/MovieLens/varying_H_k%s.tsv" % k)
    print(max_t, max_s)
    draw_index_space_H(datasets[2], t, s, k, 0, 4, 1, 0, 0, 25, 5, 0)

    t, s, max_t, max_s = read_csv_index_space("results/Music100/varying_H_k%s.tsv" % k)
    print(max_t, max_s)
    draw_index_space_H(datasets[3], t, s, k, 30, 80, 10, 0, 0, 4, 1, 2)

    t, s, max_t, max_s = read_csv_index_space("results/Netflix/varying_H_k%s.tsv" % k)
    print(max_t, max_s)
    draw_index_space_H(datasets[4], t, s, k, 0, 30, 5, 0, 0, 1.6, 0.4, 2)
