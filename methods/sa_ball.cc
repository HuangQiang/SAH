...
// -----------------------------------------------------------------------------
void SA_BALL::compute_norm_and_sort(// compute l2-norm and sort (descending)
...
// -----------------------------------------------------------------------------
void SA_BALL::blocking_user_set(    // split the user_set into blocks
    int   n0,                           // the first n0 elements in item_set
    const float *user_set)              // user_set
{
    // build a ball-tree for user_set
    tree_ = new Ball_Tree(m_, d_, leaf_, user_set);
    
    // traversal the ball-tree to get the blocks (ball-nodes) of user_set 
    blocks_.clear();
    tree_->traversal(blocks_);
    
    // build lower bounds for the users in each ball-node
...
// -----------------------------------------------------------------------------
void SA_BALL::lower_bounds_computation(// compute lower bounds for users
...
// -----------------------------------------------------------------------------
void SA_BALL::update_lower_bound(   // update lower bound
    int   k,                            // top-k value
...
// -----------------------------------------------------------------------------
void SA_BALL::node_lower_bounds_computation(// compute lower bound for a node
    int   m,                            // number of users
...
// -----------------------------------------------------------------------------
void SA_BALL::blocking_item_set(    // split the rest item_set into blocks
    int   n,                            // item cardinality
...
// -----------------------------------------------------------------------------
void SA_BALL::add_block_by_items(   // add one block by items
...
SA_BALL::~SA_BALL()                 // destructor
{
    ...
    std::vector<Ball_Node*>().swap(blocks_);
    
    if (tree_ != nullptr) { delete tree_; tree_ = nullptr; }
}

// -------------------------------------------------------------------------
void SA_BALL::display()             // display parameters
{
    printf("Parameters of SA_BALL:\n");
    printf("n             = %d\n",   n_);
    printf("m             = %d\n",   m_);
    printf("d             = %d\n",   d_);
    printf("k_max         = %d\n",   k_max_);
...
// -----------------------------------------------------------------------------
void SA_BALL::reverse_kmips(        // reverse k-mips
    int   k,                            // top k value
    ...
    std::vector<int> &result)           // reverse k-mips result (return)
{
    ...
    for (auto block : blocks_) {
        ...
        float ub = block->ball_upper_bound(ip, query_norm);
        ...
        for (int i = 0; i < m; ++i) {
            ...
            // 1.1 New Lemma: use point (user) ball upper bound for pruning
            // ub = block->ball_upper_bound(i, ip, query_norm);
            // if (ub < user_k_lb) continue; // No
            
            // 1.2 New Lemma: use point (user) cone upper bound for pruning
            ub = block->cone_upper_bound(i, q_cos, q_sin);
            if (ub < user_k_lb) continue; // No
            
            // 1.3 use lower_bound for pruning  (lemma 1)
            const float *user = user_set + (u64) i*d_;
